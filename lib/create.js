...
const initMessage = () => {
  if (fs.existsSync(i18nFile)) {
    try {
      ...
      console.log('init generate', generate)
    } catch (e) {
    }
  }
...
const getCurrentKey = (match, file) => {
  if (!messagesHash[match]) messagesHash[match] = `i18n_${match}`
  return messagesHash[match]
  // console.log('[messagesHash]', messagesHash)
  // if (messagesHash[match]) return messagesHash[match];
  // let key = getPreKey(file) + (generate += 1);
...
const generateVueFile = file => {
  ...
  content = content.replace(/<script(.|\n|\r)*script>/gim, match => {
    ...
    match = match.replace(/(['"`])(((?!\1).)*[\u4e00-\u9fa5]+((?!\1).)*)\1/gim, (_, prev, match) => {
      match = match.trim();
      let currentKey;
      let result = '';
      if (prev !== '`') {
...
const generateJsFile = (file) => {
  ...
  if (!content.match(/const[\s\t]+\$t[\s\t]+=[\s\t]+handleInjectI18n.\$t.bind\(handleInjectI18n\)[\s\t]*;?/)) {
    content = content.replace(lastImport, $ => {
      return `${$}\nlet handleInjectI18n = new ${vueModule}();\nconst $t = handleInjectI18n.$t.bind(handleInjectI18n);`;
    });
  }
  //替换注释部分
  let comments = {};
  ...
  content = content.replace(/(['"`])(((?!\1).)*[\u4e00-\u9fa5]+((?!\1).)*)\1/gim, (_, prev, match) => {
    console.log('[replace]', _, prev, match)
    match = match.trim();
    let currentKey;
    let result = '';
    if (prev !== '`') {
 ...
 * @param options
 */
module.exports.create = (src, options) => {
